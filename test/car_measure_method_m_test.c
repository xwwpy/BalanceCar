...
void car_measure_method_m_test_proc(void) {
    ...
    if (current_time - start_measure_time > M_MEASURE_SPEED_INTERVAL) {
        
        float L_angle = car_encoder_get_L_angle();
        float R_angle = car_encoder_get_R_angle();

        float L_speed = (L_angle - last_L_angle) / (M_MEASURE_SPEED_INTERVAL) * 1000.0f; // 计算速度，单位为度/秒
        float R_speed = (R_angle - last_R_angle) / (M_MEASURE_SPEED_INTERVAL) * 1000.0f; // 计算速度，单位为度/秒
